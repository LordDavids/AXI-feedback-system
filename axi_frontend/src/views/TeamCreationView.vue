...
const handleSubmit = (e: Event) => {
  ...
  createTeam(); // Call the createTeam method
};
...
<template>
  <div class="container px-0 sm:px-4">
    <div class="flex flex-col gap-2 p-6 rounded-lg shadow-lg bg-white">
      <Breadcrumbs />
      <h1 class="hidden md:hidden lg:block text-2xl font-bold mb-4 text-center">Creëer een team</h1>
      ...
      <div class="flex flex-col md:flex-row gap-4">
        <div class="flex flex-col w-full md:w-1/2">
          ...
          <ul class="min-h-80 max-h-80 overflow-y-auto flex-grow">
            <li v-for="user in selectedUsers" :key="user.id" class="p-4 bg-gray-100 rounded-lg mt-4 mb-4 flex justify-between items-center">
    <span>
      {{ user.firstName }} {{ user.infix ? user.infix + ' ' : '' }}{{ user.lastName }}
        ...
        <div class="flex flex-col w-full md:w-1/2">
          <label class="block text-gray-700 font-medium text-left mb-2 mt-2">Zoek gebruikers</label>
          <div v-if="error" class="text-red-500 mt-2">{{ error }}</div>
          <ul class="space-y-4 mt-4 min-h-72 max-h-72 overflow-y-auto flex-grow p-2">
            <input
                v-model="searchTerm"
                type="text"
                name="searchTerm"
                class="w-full px-4 py-2 pb-2 border rounded-lg text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400"
                placeholder="Zoeken..."
                required
            >
            ...
            <li
                ...
                :key="user.id"
                class="p-4 border border-gray-300 rounded-lg hover:bg-gray-50 cursor-pointer"
