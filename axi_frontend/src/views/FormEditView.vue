...
const editForm = async () => {
  ...
  await api.put(`/team/${id}/form/${formId}/update`, {
    name: formName.value,
    ...
    isPublic: isPublic.value
      ...
      .then((response) => {
        router.push(`/team/${id}/form/${formId}`);
      })
      .catch((err) => {
...
const fetchFormDetails = async () => {
  ...
  await api.get(`/team/${id}/form/${formId}`)
      .then((response) => {
        ...
        formName.value = form.value.name;
        selectedCategory.value = form.value.category.id;
        selectedQuestions.value = form.value.questions;
